...
def updateStockbasicToDB():
    ...
    try:
        if not (_deleteTableInfo(connectSQLite.STOCKBAISCTABLE)):
...
def updateCompanyInfoToDB():
    ...
    try:
        if not (_deleteTableInfo(connectSQLite.COMPANYTABLE)): return
...
def getAllStockDailyInfo():
    _STARTTIME = _getMaxdateFromTable(connectSQLite.DAILYTABLE)
...
def getAllTopListInfo():
    _STARTTIME = _getMaxdateFromTable(connectSQLite.TOPLISTTABLE)
    if _STARTTIME >= _NOWTIME:
        print('龙虎榜当前信息已是最新')
        return
    #需要将20190101格式字符串转换为时间格式，便于循环
...
def getAllTopInstInfo():
    _STARTTIME = _getMaxdateFromTable(connectSQLite.TOPINSTTABLE)
    if _STARTTIME >= _NOWTIME:
...
def getAllBlockTradeInfo():
    _STARTTIME = _getMaxdateFromTable(connectSQLite.BLOCKTRADETABLE)
    if _STARTTIME >= _NOWTIME:
        print('大宗交易当前信息已是最新')
        return
    #需要将20190101格式字符串转换为时间格式，便于循环
...
def getAllStockDailyBasicInfo():
    _STARTTIME = _getMaxdateFromTable(connectSQLite.DAILYBASICTABLE)
    if _STARTTIME >= _NOWTIME:
...
def _getMaxdateFromTable(tablename):
    ...
    try:
        ...
        result = connectSQLite.sqliteCon.fetchone()
        if result[0] == None:
            return _STARTTIME
