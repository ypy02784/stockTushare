...
def initial_model_header_dict():
    sql = 'select * from table_info'
    DBCur.execute(sql)
    df = DBCur.fetchall()
    for each in df:
        MODEL_HEADER_DICT[each[2]] = each[4]
    # print(MODEL_HEADER_DICT)
    # print(len(MODEL_HEADER_DICT))   #目前20190322共有92个不同的列


initial_model_header_dict()


# description为select语句执行后记录数据列的元组，df为数据集list
def setModel(df=[], description=()):
    ...
    row = len(df)  # df的长度决定行数
    vol = len(description)  # description的长度决定列数
    model.setRowCount(row)
    model.setColumnCount(vol)

    i = 0
    for header in description:
        model.setHorizontalHeaderItem(i, QtGui.QStandardItem(MODEL_HEADER_DICT[header[0]]))  # 根据列名设置model标题
        i += 1
