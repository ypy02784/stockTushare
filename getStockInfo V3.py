...
stockDB = pymysql.connect("localhost","root","",DBname,charset='utf8')# 打开数据库连接  使用mysql连接数库
cursorDB = stockDB.cursor() # 使用 cursor() 方法创建一个游标对象 cursor
cn = create_engine('mysql+pymysql://root:@localhost:3306/'+DBname+'?charset=utf8')#创建数据引擎方便批量插入数据
...
def updateStockbasicToDB():
    ...
    try:
        if not(deleteTableInfo(STOCKBAISCTABLE)) : return#删除表中数据失败则推出函数，避免重复数据
        stock_basic.to_sql(STOCKBAISCTABLE,cn,index=False,if_exists='append')
